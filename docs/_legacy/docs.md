...
We sign all requests using your secret and the new `oauth_token` and `oauth_token_secret`. We're talking standard HMAC-SHA1, RSA is not supported. However, you can use [scripting]({{ base_url }}scripting) to modify request parameters (including headers) as you see fit.
...
> **A complete OAuth V2 example is available on our [Formstack example]({{ base_url }}rest-hooks-example) page.** Check that out for more detail in a real world example!
...
> **Hint:** Does your app need to change the URL based on the user's subdomain or domain? Simply add another authentication field (right alongside `access_token` from the above screenshot) that has the key like `subdomain` and then use it like `https://{% templatetag openvariable %}subdomain{% templatetag closevariable %}.yourwebsite.com/api/token` in all your URLs.
...
By default, we append the `access_token` to the querystring for all requests, as well as in an Authorization header, however, you can use [scripting]({{ base_url }}scripting) to modify request parameters as you see fit (IE: place the `access_token` in a special header). This is called the `bearer` token type.
...
If your API does not signal expired tokens with a `401`, use [scripting]({{ base_url }}scripting) to manually set the status code header to `401` upon conditions of your choosing. Or, you can raise a `RefreshTokenException` in your `post_XXXX` function call to kick off a new refresh.
...
Additionally, these user-provided variables will be passed in with the `bundle` for usage in [scripting]({{ base_url }}scripting). For example, the above screenshots would translate to:
...
This is somewhat arbitrary but you should pick a good, short key name because you'll need to include it elsewhere in the developer interface as `{% templatetag openvariable %}key{% templatetag closevariable %}` using the [variable syntax](#variable-syntax). Needs to be at least two characters long, start with an alpha, and only contain `a-z`, `A-Z`, `0-9` or `_`.
...
Example: <br/> `https://,{% templatetag openvariable %}input{% templatetag closevariable %},.yourdomain.com/` will create:
...
Also, it is important to know that by default, we do our best to inspect and pull out all relevant information in a request. If you use [scripting]({{base_url }}scripting) to further refine the hook data, you'll see it passed in as `bundle.cleaned_request`. For example, a request might have a `cleaned_request` similar to:
...
Some webhooks send single objects, while some are batched as lists of objects for efficiency reasons. Though Zapier supports both, we highly recommend using [scripting]({{ base_url }}scripting) if you are dealing with batched lists. We do our best via `cleaned_request` to interpret lists where possible, but if your list is hidden in a subkey, we won't capture it properly.
...
This is where you'd need to break out [scripting]({{ base_url }}scripting) to do some refinement. Check out the examples we provide in the [scripting examples]({{ base_url }}scripting/#catching-webhooks-example).
...
_Though we highly recommend this native pattern, you can override all the methods below with [scripting]({{ base_url }}scripting). This means you can bend our pattern to match an existing pattern of your own._
...
[Some example code is provided to make this as easy as possible.]({{ base_url }}scripting/#rest-hook-subscription-examples) A heads up: you will need to use that code if you intend to do a DELETE call. You'll be able to access `bundle.subscribe_data` to build the URL in `pre_unsubscribe`. The default behavior is slightly different and listed below.
...
This is a field only really used internally for both [dynamic dropdown](#dynamic-dropdowns) and [scripting]({{ base_url }}scripting) references. Needs to be at least two characters long, start with an alpha, and only contain `a-z`, `A-Z`, `0-9` or `_`.
...
Example: <br/> `https://example.com/api/v1/users/{% templatetag openvariable %}id{% templatetag closevariable %}/contacts.json?sort=desc&on=date_created`

#### A Warning about encoding URL params

We will not automatically encode any URL variables, so you're responsible for encoding any if they require that. For example, emails _might_ include a `+` sign, so if you have `https://example.com/api/v1/users?email={% templatetag openvariable %}email{% templatetag closevariable %}` you'll want to encode that in your [`TRIGGER_KEY_pre_poll`](#available-methods/#polling) (or remove it from there and add it to the `bundle.request.params`), otherwise you'll get a "space" where the `+` sign is.
...
Example: <br/> `http://api.example.com/v2/fields.json` or `http://{% templatetag openvariable %}account{% templatetag closevariable %}.example.com/api/v1/fields.json`
...
> Read more about [custom field formatting here]({{ base_url }}trigger-fields-custom).
...
A key for your internal use. It is available for [variable syntax](#variable-syntax) in the Trigger URL field (as well as in [scripting]({{ base_url }}scripting)). Needs to be at least two characters long, start with an alpha, and only contain `a-z`, `A-Z`, `0-9` or `_`.
...
*   Ensure the URL route returns data in the below format, or manipulate to fit with [a `_post_custom_trigger_fields` scripting method]({{ base_url }}scripting/#key_post_custom_trigger_fields).
...
This is a field only really used internally for both [dynamic dropdowns]({{ base_url}}dynamic-dropdowns) and [scripting](#scripting) references. Needs to be at least two characters long, start with an alpha, and only contain `a-z`, `A-Z`, `0-9` or `_`.
...
Define the URL where we will, by default, `POST` the payload to. You can also make use of [variable syntax](#variable-syntax) where [auth fields]({{ base_url }}authentication-fields) and [action fields]({{ base_url }}action-fields) will be injected.

Example: <br/> `http://api.example.com/v2/clients.json` or `http://{% templatetag openvariable %}account{% templatetag closevariable %}.example.com/api/v1/projects.json`

> What about the payload? You can define those in [action fields]({{ base_url }}action-fields), anything you put there and check "Send in JSON?" will be included in the JSON according to its key!

#### A Warning about encoding URL params

We will not automatically encode any URL variables, so you're responsible for encoding any if they require that. For example, emails _might_ include a `+` sign, so if you have `https://example.com/api/v2/clients.json?email={% templatetag openvariable %}email{% templatetag closevariable %}` you'll want to encode that in your [`ACTION_KEY_pre_write`](#available-methods/#action-methods) (or remove it from there and add it to the `bundle.request.params`), otherwise you'll get a "space" where the `+` sign is.

A better approach is to not even include it in the URL (it'll be added and encoded automatically in that case)

### Custom Action Fields URL

This allows you to dynamically define action fields that are user set (IE: custom fields). They get passed into the POST'ed JSON just like normal action fields (or into [scripting]({{base_url }}scripting)).

Example: <br/> `http://api.example.com/v2/fields.json` or `http://{% templatetag openvariable %}account{% templatetag closevariable %}.example.com/api/v1/fields.json`

> Read more about [custom field formatting here]({{ base_url }}action-fields-custom).
...
You can provide a `choices` array which will be mapped automatically into a dropdown in the Zap Editor. In the developer UI you should simply provide us with a hard-coded list of comma-separated values for the "choices" option when defining a field. If you're using [scripting]({{ base_url }}scripting), you'll want to provide these choices to us manually. Here is a code sample and what it looks like:
...
You can make a field a list by checking the appropriate box in the UI. For custom fields, you can use [Scripting]({{ base_url }}scripting) to alter the field definition to include `list: true` like this:
...
* Ensure the URL route returns data in the below format, or manipulate it to fit with [scripting]({{ base_url }}scripting/#key_custom_action_fields).
...
* Ensure that the URL route returns data in the below format, or manipulate it to fit with [scripting]({{ base_url }}scripting). Extra data will be ignored, but we require at least the following to properly format your action results.
...
(This works similar to [trigger sample results](https://zapier.com/developer/documentation/v2/trigger-sample-results))
...
We will not automatically encode any URL variables, so you're responsible for encoding any if they require that. For example, emails _might_ include a `+` sign, so if you have `https://example.com/api/v2/leads?email={% templatetag openvariable %}email{% templatetag closevariable %}` you'll want to encode that in your [`SEARCH_KEY_pre_search`](#available-methods/#search-methods) (or remove it from there and add it to the `bundle.request.params`), otherwise you'll get a "space" where the `+` sign is.
...
> Read more about [custom field formating here]({{ base_url }}search-fields-custom).
...
You can make use of [variable syntax](#variable-syntax) where [auth fields]({{ base_url }}auth-fields) and search result fields (not the actual [search fields]({{ base_url }}search-fields)) will be injected.
...
And you configured this field with `https://api.example.com/v2/leads/{% templatetag openvariable %}id{% templatetag closevariable %}.json` then we'd GET `https://api.example.com/v2/leads/124.json` to get the full result of the found or created item.
...
After every successful Search or Create - we'll attempt to grab a fresh record via the [Resource URL]({{ base_url }}searches/#resource-url).
...
* Ensure the URL route returns data in the below format, or manipulate it to fit with  [Scripting]({{ base_url }}scripting).
* You can choose from several internal types, documented here: [Field Types](#field-types).
...
- Ensure that the URL route returns data in the below format, or manipulate it to fit with  [Scripting]({{ base_url }}scripting). Extra data will be ignored, but we require at least the following to properly format your search action results.
...
## How to Launch Your Zapier Integration
...
The keys to put between the {% templatetag openvariable %}{% templatetag closevariable %} come from the keys you specify when you setup [action fields](#action-fields), [trigger fields](#trigger-fields), and [authentication fields](#authentication-fields).

***

<a id="versions"></a>
## Legacy Web Builder Versions Changelist
