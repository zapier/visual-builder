...
As triggers only watch for new data and typically need to send no or little data to the app, they're often quicker to set up than Zapier [actions](https://platform.zapier.com/build/action). Zapier can watch for any new or updated item through your API—or optionally, you can include [input fields](https://platform.zapier.com/build/input-designer) for users to enter filters, tags, and other details to filter through new data and watch for the items they want.
...
In the "Input Designer" tab of the visual builder's trigger settings, add input fields for each piece of data you need from users to configure the trigger. Check our [Input Designer Guide](https://platform.zapier.com/build/input-designer) for complete details on how to create an input form and each of the field types and options available.
...
Polling API results are expected to be an array with 0 or more objects that will be passed to Zapier's [deduper](https://platform.zapier.com/build/dedupe). The deduper will return any items that haven't been received before, and use them to run the subsequent steps in the user's Zap.
...
Alternately, if your app supports REST Hooks—webhook subscriptions that can be manipulated through a REST API—select _REST Hook_ for your trigger. Note that the Zapier implementation does not support Identity Confirmation.
