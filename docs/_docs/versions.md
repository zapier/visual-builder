...
* [Zap templates](https://platform.zapier.com/partners/zap-templates) are updated to use the new Public version of your app if there are no breaking changes
...
Several change scenarios are valiated by the platform when you try to "Migrate" after a version promotion, but always be aware of the affects of any changes you make before you even begin implementing those changes.
...
> **Note:** You can also set a deprecation date using the CLI, with the [`zapier deprecate`](https://github.com/zapier/zapier-platform/blob/master/packages/cli/docs/cli.md#deprecate) command. [More about that here](https://github.com/zapier/zapier-platform/blob/main/packages/cli/README.md#promoting-an-app-version).
